...
def test_loads_cases_sorted_by_filename(tmp_path):
    ...
    assert cases[0] == Case(name="01-a", input="q1", expected="a1", metadata={"category": "x"})
    assert cases[1].metadata == {}


def test_picks_up_yml_extension(tmp_path):
...
def test_missing_expected_raises(tmp_path):
