...
def test_high_non_anthropic_uses_reasoning_effort(monkeypatch):
    captured = _patch_completion(monkeypatch)
    cfg = RunConfig(dataset_dir="d", model="openai/gpt-5", effort="high", temperature=0.2)
    model.make_task(cfg)({"question": "Q"})
...
def test_none_effort_sends_no_reasoning_params(monkeypatch):
    captured = _patch_completion(monkeypatch)
...
def _fake_judge(label):
    judge = types.SimpleNamespace()
    judge.generate_object = lambda prompt, schema: {"label": label, "explanation": "because"}
    return judge


def test_judge_positive_label_scores_one():
...
def test_judge_negative_label_scores_zero():
    faith = next(s for s in evaluators._SPECS if s.name == "faithfulness")
    fn = evaluators._build_eval_fn(faith, judge=_fake_judge("unfaithful"), offline=False)
